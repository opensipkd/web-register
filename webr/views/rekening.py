...
def query_reg():
    return DBSession.query(Rekening.kode, Rekening.nama).order_by(Rekening.kode)
    
########                    
# CSV #
########          
@view_config(route_name='rekening-csv', renderer='csv')
...
# PDF    #
##########    
@view_config(route_name='rekening-pdf', permission='read')
def view_pdf(request):
    params   = request.params
    url_dict = request.matchdict
    if url_dict['pdf']=='reg' :
        query = query_reg()
        rml_row = open_rml_row('rekening.row.rml')
        rows=[]
        for r in query.all():
            s = rml_row.format(kode=r.kode, nama=r.nama)
            rows.append(s)
            
        pdf, filename = open_rml_pdf('rekening.rml', rows=rows)
